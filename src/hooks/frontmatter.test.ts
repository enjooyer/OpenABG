...
`;
    ...
    expect(parsed.openabg.emoji).toBe("💾");
    expect(parsed.openabg.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.openabg.emoji).toBe("📝");
    expect(parsed.openabg.events).toEqual(["command"]);
    expect(parsed.openabg.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openabg.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"openabg": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"openabg": {"events": ["test"]}}');
  });
...
describe("resolveOpenABGMetadata", () => {
  it("extracts openabg metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveOpenABGMetadata(frontmatter);
  ...
  it("returns undefined when openabg key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openabg: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with OpenABG" },
            { id: "npm", kind: "npm", package: "@openabg/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openabg/hook");
  });
...
homepage: https://docs.openabg.ai/hooks#session-memory
metadata:
  {
    "openabg":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with OpenABG" }],
      },
  }
---
...
`;
    ...
    const openabg = resolveOpenABGMetadata(frontmatter);
    expect(openabg).toBeDefined();
    expect(openabg?.emoji).toBe("💾");
    expect(openabg?.events).toEqual(["command:new"]);
    expect(openabg?.requires?.config).toEqual(["workspace.dir"]);
    expect(openabg?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const openabg = resolveOpenABGMetadata(frontmatter);
    expect(openabg?.emoji).toBe("disk");
    expect(openabg?.events).toEqual(["command:new"]);
  });
});
