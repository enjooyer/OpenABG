...
vi.mock("./chrome.js", () => ({
  ...
  launchOpenABGChrome: vi.fn(async () => {
  ...
  return {
    ...
    resolved: {
      ...
      profiles: {
        ...
        openabg: { cdpPort: 18800, color: "#FF4500" },
      },
    },
    profiles: new Map(),
  };
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const { createBrowserRouteContext } = await import("./server-context.js");
    const state = makeState("openabg");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const openabg = ctx.forProfile("openabg");

    const opened = await openabg.openTab("https://created.example");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("openabg")?.lastTargetId).toBe("CREATED");
  });
});
