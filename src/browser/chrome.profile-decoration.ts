...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openabg-profile-decorated");
}
 ...
 */
export function decorateOpenABGProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENABG_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENABG_BROWSER_COLOR).toUpperCase();
