import type { BrowserConfig, BrowserProfileConfig, OpenABGConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENABG_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPENABG_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openabg" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENABG_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENABG_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenABG browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openabg/openabg CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenABGConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENABG_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openabg";
