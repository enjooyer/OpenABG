...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.OPENABG_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.OPENABG_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.OPENABG_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[openabg] ${parsed.error}`);
    process.exit(2);
  }
