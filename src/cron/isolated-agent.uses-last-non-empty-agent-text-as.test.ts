...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "openabg-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".openabg", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenABGConfig> = {},
): OpenABGConfig {
  const base: OpenABGConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "openabg"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".openabg", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
