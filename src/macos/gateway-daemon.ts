...
declare const __OPENABG_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __OPENABG_VERSION__ === "string" && __OPENABG_VERSION__) ||
  process.env.OPENABG_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `openabg --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.OPENABG_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.OPENABG_GATEWAY_BIND ??
