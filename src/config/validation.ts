...
import type { OpenABGConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenABGConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: OpenABGConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenABGSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenABGConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenABGConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as OpenABGConfig)),
    ),
  };
}
