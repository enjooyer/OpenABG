...
import type { OpenABGConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
import { compareOpenABGVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "OPENABG_GATEWAY_TOKEN",
  "OPENABG_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): OpenABGConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OpenABGConfig;
}
...
function stampConfigVersion(cfg: OpenABGConfig): OpenABGConfig {
...
function warnIfConfigFromFuture(cfg: OpenABGConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenABGVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenABG (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: OpenABGConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenABGConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as OpenABGConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenABGConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenABGConfig) {
    clearConfigCache();
...
// module scope. `OPENABG_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENABG_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENABG_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): OpenABGConfig {
...
export async function writeConfigFile(cfg: OpenABGConfig): Promise<void> {
  clearConfigCache();
