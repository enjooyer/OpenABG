...
const ETC_OPENABG_DIR = path.join(ROOT_DIR, "etc", "openabg");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "openabg.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcOpenABGPath(...parts: string[]) {
  return path.join(ETC_OPENABG_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcOpenABGPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "openabg.json"))).toEqual({
      shared: true,
    });
  });
});
