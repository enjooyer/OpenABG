import type { OpenABGConfig } from "./config.js";
...
function isTelegramConfigured(cfg: OpenABGConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: OpenABGConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: OpenABGConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: OpenABGConfig): boolean {
...
function isIMessageConfigured(cfg: OpenABGConfig): boolean {
...
function isWhatsAppConfigured(cfg: OpenABGConfig): boolean {
...
function isGenericChannelConfigured(cfg: OpenABGConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: OpenABGConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OpenABGConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: OpenABGConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: OpenABGConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: OpenABGConfig, pluginId: string): OpenABGConfig {
...
function registerPluginEntry(cfg: OpenABGConfig, pluginId: string): OpenABGConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
