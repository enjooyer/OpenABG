...
function setWhatsAppDmPolicy(cfg: OpenABGConfig, dmPolicy: DmPolicy): OpenABGConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: OpenABGConfig, allowFrom?: string[]): OpenABGConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: OpenABGConfig, selfChatMode: boolean): OpenABGConfig {
...
async function detectWhatsAppLinked(cfg: OpenABGConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so OpenABG can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for OpenABG" },
    ],
  });
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("openabg channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
