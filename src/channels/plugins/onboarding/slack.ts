...
function setSlackDmPolicy(cfg: OpenABGConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "OpenABG";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for OpenABG`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/openabg",
          description: "Send a message to OpenABG",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: OpenABGConfig, allowFrom: string[]): OpenABGConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
