...
import { ensureOpenABGCliOnPath } from "./path-env.js";

describe("ensureOpenABGCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling openabg exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "openabg");
      await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(cliPath, 0o755);

      const originalPath = process.env.PATH;
      const originalFlag = process.env.OPENABG_PATH_BOOTSTRAPPED;
      process.env.PATH = "/usr/bin";
      delete process.env.OPENABG_PATH_BOOTSTRAPPED;
      try {
        ...
        if (originalFlag === undefined) {
          ...
          process.env.OPENABG_PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "openabg");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "openabg");
