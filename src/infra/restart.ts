...
export function triggerOpenABGRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENABG_SYSTEMD_UNIT,
        process.env.OPENABG_PROFILE,
      );
  ...
  const label =
    process.env.OPENABG_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENABG_PROFILE);
