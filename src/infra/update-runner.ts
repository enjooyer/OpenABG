...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "openabg.mjs");
    ...
    const doctorStep = await runStep(
      step("openabg doctor", doctorArgv, gitRoot, { OPENABG_UPDATE_IN_PROGRESS: "1" }),
    );
