...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-run-node-"));
  try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        await fs.writeFile(
          path.join(tmp, "openabg.mjs"),
          "#!/usr/bin/env node\nif (process.argv.includes('--version')) console.log('9.9.9-test');\n",
          "utf-8",
        );
        await fs.chmod(path.join(tmp, "openabg.mjs"), 0o755);
...
`;
        ...
        const env = {
          ...
          OPENABG_FORCE_BUILD: "1",
          OPENABG_RUNNER_LOG: "0",
        };
