...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENABG_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openabg-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openabg";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openabg");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openabg-gw");
  ...
  records.push(`_openabg-gw._tcp IN PTR ${instanceLabel}._openabg-gw._tcp`);
  records.push(`${instanceLabel}._openabg-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openabg-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openabg-content-hash: ${contentHash}\n${contentBody}`;
}
