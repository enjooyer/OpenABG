...
function candidateBinDirs(opts: EnsureOpenABGPathOpts): string[] {
  ...
  // Bundled macOS app: `openabg` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openabg");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/openabg` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "openabg"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openabg` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenABGCliOnPath(opts: EnsureOpenABGPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENABG_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.OPENABG_PATH_BOOTSTRAPPED = "1";
