...
import { discoverOpenABGPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpenABGPluginDefinition;
  register?: OpenABGPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpenABGPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpenABGPluginDefinition;
...
export function loadOpenABGPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOpenABGPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "openabg/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OpenABGPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as OpenABGPluginModule;
    } catch (err) {
