...
import { loadOpenABGPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.OPENABG_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openabg-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openabg.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OPENABG_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOpenABGPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OPENABG_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadOpenABGPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOpenABGPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@openabg/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        openabg: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
