...
const execHostEnforced = process.env.OPENABG_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.OPENABG_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureOpenABGCliOnPath({ pathEnv: process.env.PATH ?? "" });
