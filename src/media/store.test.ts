...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "OPENABG_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.OPENABG_STATE_DIR = path.join(home, ".openabg");
    ...
    await fs.mkdir(path.join(home, ".openabg"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.openabg${path.sep}media`);
