...
export function listAgentEntries(cfg: OpenABGConfig): AgentEntry[] {
...
function resolveAgentName(cfg: OpenABGConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenABGConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenABGConfig): AgentSummary[] {
