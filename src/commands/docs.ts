...
const SEARCH_TOOL = "https://docs.openabg.ai/mcp.SearchOpenABG";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openabg.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openabg docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openabg.ai/");
      runtime.log(`Search: ${formatCliCommand('openabg docs "your query"')}`);
    }
    return;
  }
