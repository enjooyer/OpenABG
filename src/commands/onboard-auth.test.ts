...
const requireAgentDir = () => {
  const agentDir = process.env.OPENABG_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENABG_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.OPENABG_STATE_DIR;
  const previousAgentDir = process.env.OPENABG_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.OPENABG_AGENT_DIR;
    } else {
      process.env.OPENABG_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.OPENABG_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPENABG_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-oauth-"));
    process.env.OPENABG_STATE_DIR = tempStateDir;
    process.env.OPENABG_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENABG_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENABG_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-minimax-"));
    process.env.OPENABG_STATE_DIR = tempStateDir;
    process.env.OPENABG_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENABG_AGENT_DIR;
