...
import type { OpenABGConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenABGConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openabg security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openabg configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openabg config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openabg doctor --fix")} to generate a token`,
              `  Or set token directly: ${formatCliCommand(
                "openabg config set gateway.auth.mode token",
              )}`,
            ];
      warnings.push(
        `- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`,
        `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`,
        `  Fix: ${formatCliCommand("openabg config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
