...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-onboard-"));
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "openabg");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.OPENABG_STATE_DIR = stateDir;
    process.env.OPENABG_CONFIG_PATH = path.join(stateDir, "openabg.json");
