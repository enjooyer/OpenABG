...
beforeEach(() => {
  ...
  resolveOpenABGPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENABG_STATE_DIR;
  originalUpdateInProgress = process.env.OPENABG_UPDATE_IN_PROGRESS;
  process.env.OPENABG_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openabg-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENABG_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENABG_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENABG_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveOpenABGPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenABGPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/openabg-root.js", () => ({
  resolveOpenABGPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "openabg-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.OPENABG_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when OPENABG_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/openabg.json",
      exists: true,
      raw: "{}",
    ...
    process.env.OPENABG_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
