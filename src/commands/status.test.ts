...
beforeAll(() => {
  previousProfile = process.env.OPENABG_PROFILE;
  process.env.OPENABG_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.OPENABG_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/openabg-root.js", () => ({
  resolveOpenABGPackageRoot: vi.fn().mockResolvedValue("/tmp/openabg"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openabg/pnpm-lock.yaml",
      markerPath: "/tmp/openabg/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("OpenABG status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("openabg status --all") ||
          l.includes("openabg --profile isolated status --all") ||
          l.includes("openabg status --all") ||
          l.includes("openabg --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.OPENABG_GATEWAY_TOKEN = "abcd1234";
    try {
