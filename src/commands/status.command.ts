...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenABG status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openabg security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openabg security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openabg.ai/faq");
  runtime.log("Troubleshooting: https://docs.openabg.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openabg status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openabg logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openabg status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openabg gateway probe")}`);
  }
}
