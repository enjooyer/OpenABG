...
function applyDefaultModel(cfg: OpenABGConfig, model: string): OpenABGConfig {
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveOpenABGAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
