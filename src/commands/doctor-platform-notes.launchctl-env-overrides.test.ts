...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENABG_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENABG_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENABG_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENABG_GATEWAY_PASSWORD");
  });
