...
function resolveMode(cfg: OpenABGConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenABG doctor");

  const root = await resolveOpenABGPackageRoot({
  ...
  let cfg: OpenABGConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("openabg configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openabg config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openabg setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openabg doctor --fix")}" to apply changes.`);
  }
