...
export async function updateConfig(
  mutator: (cfg: OpenABGConfig) => OpenABGConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenABGConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenABGConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openabg agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
