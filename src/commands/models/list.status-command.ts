...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenABGAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openabg models auth setup-token")}\` or \`${formatCliCommand("openabg configure")}\`.`
          : `Run \`${formatCliCommand("openabg configure")}\` or set an API key env var.`;
