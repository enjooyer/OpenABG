...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenABGAgentDir: vi.fn().mockReturnValue("/tmp/openabg-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openabg-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openabg-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenABGAgentDir: mocks.resolveOpenABGAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenABGAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/openabg-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/openabg-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/openabg-agent-custom");
