...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-commands-"));
...
function buildParams(commandBody: string, cfg: OpenABGConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
