...
import type { OpenABGConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: OpenABGConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenABGConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenABGConfig),
...
export function getCommandDetection(_cfg?: OpenABGConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenABGConfig) {
