...
async function withTempConfig(params: { cfg: unknown; run: () => Promise<void> }): Promise<void> {
  const prevConfigPath = process.env.OPENABG_CONFIG_PATH;
  const prevDisableCache = process.env.OPENABG_DISABLE_CONFIG_CACHE;

  const dir = await mkdtemp(path.join(os.tmpdir(), "openabg-canvas-auth-test-"));
  const configPath = path.join(dir, "openabg.json");

  process.env.OPENABG_CONFIG_PATH = configPath;
  process.env.OPENABG_DISABLE_CONFIG_CACHE = "1";
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.OPENABG_CONFIG_PATH = prevConfigPath;
    }
    if (prevDisableCache === undefined) {
      delete process.env.OPENABG_DISABLE_CONFIG_CACHE;
    } else {
      process.env.OPENABG_DISABLE_CONFIG_CACHE = prevDisableCache;
    }
