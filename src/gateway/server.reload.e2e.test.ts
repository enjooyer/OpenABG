...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.OPENABG_SKIP_CHANNELS;
    prevSkipGmail = process.env.OPENABG_SKIP_GMAIL_WATCHER;
    process.env.OPENABG_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.OPENABG_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.OPENABG_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENABG_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
