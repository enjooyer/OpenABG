...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.OPENABG_STATE_DIR;
  previousConfigPath = process.env.OPENABG_CONFIG_PATH;
  previousSkipBrowserControl = process.env.OPENABG_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.OPENABG_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.OPENABG_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.OPENABG_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENABG_STATE_DIR = path.join(tempHome, ".openabg");
  delete process.env.OPENABG_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPENABG_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPENABG_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENABG_SKIP_CANVAS_HOST = "1";
  process.env.OPENABG_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "openabg-test-no-bundled-extensions")
    : "openabg-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "openabg-test-"))
    : path.join(tempHome, ".openabg-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.OPENABG_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.OPENABG_CONFIG_PATH;
    } else {
      process.env.OPENABG_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.OPENABG_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.OPENABG_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.OPENABG_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENABG_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.OPENABG_SKIP_CANVAS_HOST;
    } else {
      process.env.OPENABG_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.OPENABG_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.OPENABG_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.OPENABG_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENABG_GATEWAY_TOKEN = fallbackToken;
  }
