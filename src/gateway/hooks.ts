...
export function resolveHooksConfig(cfg: OpenABGConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-openabg-token"] === "string"
      ? req.headers["x-openabg-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
