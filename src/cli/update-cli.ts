...
const STEP_LABELS: Record<string, string> = {
  ...
  "openabg doctor entry": "Checking doctor entrypoint",
  "openabg doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "openabg";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const OPENABG_REPO_URL = "https://github.com/openabg/openabg.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), ".openabg");
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("openabg@")) {
    return trimmed.slice("openabg@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "openabg.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("openabg completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.OPENABG_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENABG_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENABG_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENABG_GIT_DIR to an empty folder or an openabg checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENABG_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveOpenABGPackageRoot({
  ...
  defaultRuntime.log(theme.heading("OpenABG update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating OpenABG..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this OpenABG install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("openabg doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("openabg gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g openabg@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g openabg@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.OPENABG_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.OPENABG_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("openabg gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openabg doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("openabg gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openabg gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `openabg update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OPENABG_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENABG_GIT_DIR to an empty folder or an openabg checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via OPENABG_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OpenABG to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["openabg update", "Update a source checkout (git)"],
        ["openabg update --channel beta", "Switch to beta channel (git + npm)"],
        ["openabg update --channel dev", "Switch to dev channel (git + npm)"],
        ["openabg update --tag beta", "One-off update to a dist-tag or version"],
        ["openabg update --no-restart", "Update without restarting the service"],
        ["openabg update --json", "Output result as JSON"],
        ["openabg update --yes", "Non-interactive (accept downgrade prompts)"],
        ["openabg update wizard", "Interactive update wizard"],
        ["openabg --update", "Shorthand for openabg update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run openabg update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openabg.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openabg update status", "Show channel + version status."],
          ["openabg update status --json", "JSON output."],
          ["openabg update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.openabg.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
