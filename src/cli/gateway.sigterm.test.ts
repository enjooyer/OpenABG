...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openabg-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      OPENABG_NO_RESPAWN: "1",
      OPENABG_STATE_DIR: stateDir,
      OPENABG_SKIP_CHANNELS: "1",
      OPENABG_SKIP_GMAIL_WATCHER: "1",
      OPENABG_SKIP_CRON: "1",
      OPENABG_SKIP_BROWSER_CONTROL_SERVER: "1",
      OPENABG_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "openabg-entry-bootstrap.mjs");
