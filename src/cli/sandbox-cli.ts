...
const SANDBOX_EXAMPLES = {
  main: [
    ["openabg sandbox list", "List all sandbox containers."],
    ["openabg sandbox list --browser", "List only browser containers."],
    ["openabg sandbox recreate --all", "Recreate all containers."],
    ["openabg sandbox recreate --session main", "Recreate a specific session."],
    ["openabg sandbox recreate --agent mybot", "Recreate agent containers."],
    ["openabg sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["openabg sandbox list", "List all sandbox containers."],
    ["openabg sandbox list --browser", "List only browser containers."],
    ["openabg sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["openabg sandbox recreate --all", "Recreate all containers."],
    ["openabg sandbox recreate --session main", "Recreate a specific session."],
    ["openabg sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["openabg sandbox recreate --browser --all", "Recreate only browser containers."],
    ["openabg sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["openabg sandbox explain", "Show effective sandbox config."],
    ["openabg sandbox explain --session agent:main:main", "Explain a specific session."],
    ["openabg sandbox explain --agent work", "Explain an agent sandbox."],
    ["openabg sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.openabg.ai/cli/sandbox")}\n`,
    )
    .action(() => {
