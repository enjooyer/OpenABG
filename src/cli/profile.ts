...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.openabg${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.OPENABG_PROFILE = profile;

  const stateDir = env.OPENABG_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.OPENABG_STATE_DIR?.trim()) {
    env.OPENABG_STATE_DIR = stateDir;
  }

  if (!env.OPENABG_CONFIG_PATH?.trim()) {
    env.OPENABG_CONFIG_PATH = path.join(stateDir, "openabg.json");
  }

  if (profile === "dev" && !env.OPENABG_GATEWAY_PORT?.trim()) {
    env.OPENABG_GATEWAY_PORT = "19001";
  }
}
