...
${formatHelpExamples([
  ['openabg agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openabg agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openabg agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openabg agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openabg agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openabg agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openabg.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openabg.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openabg agents set-identity --agent main --name "OpenABG" --emoji "🦞"', "Set name + emoji."],
  ["openabg agents set-identity --agent main --avatar avatars/openabg.png", "Set avatar path."],
  [
    "openabg agents set-identity --workspace ~/.openabg/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "openabg agents set-identity --identity-file ~/.openabg/workspace/IDENTITY.md --agent main",
