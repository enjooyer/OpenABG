...
import { __testing, createOpenABGCodingTools } from "./pi-tools.js";
...
describe("createOpenABGCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createOpenABGTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOpenABGCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "openabg-outside.txt");
