...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openabg.sandbox=1");
  args.push("--label", `openabg.sessionKey=${params.scopeKey}`);
  args.push("--label", `openabg.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openabg.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("openabg.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openabg sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openabg sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openabg sandbox recreate --all");
}
