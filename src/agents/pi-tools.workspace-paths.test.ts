...
import { createOpenABGCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("openabg-ws-", async (workspaceDir) => {
      await withTempDir("openabg-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpenABGCodingTools({ workspaceDir });
    ...
    await withTempDir("openabg-ws-", async (workspaceDir) => {
      await withTempDir("openabg-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello openabg");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("openabg-ws-", async (workspaceDir) => {
      const tools = createOpenABGCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openabg-ws-", async (workspaceDir) => {
      await withTempDir("openabg-override-", async (overrideDir) => {
        const tools = createOpenABGCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openabg-sandbox-", async (sandboxDir) => {
      await withTempDir("openabg-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "openabg-sbx-test",
          containerWorkdir: "/workspace",
          docker: {
            image: "openabg-sandbox:bookworm-slim",
            containerPrefix: "openabg-sbx-",
            workdir: "/workspace",
            readOnlyRoot: true,
            tmpfs: [],
            network: "none",
        ...
        const tools = createOpenABGCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
