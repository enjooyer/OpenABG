...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-embedded-agent-"));
...
const ensureModels = (cfg: OpenABGConfig) => ensureOpenABGModelsJson(cfg, agentDir) as unknown;
