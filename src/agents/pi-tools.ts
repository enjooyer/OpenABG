...
function resolveExecConfig(cfg: OpenABGConfig | undefined) {
...
export function createOpenABGCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createOpenABGReadTool(freshReadTool)];
    }
