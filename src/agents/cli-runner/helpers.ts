...
/**
 * Cleanup suspended OpenABG CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OpenABGConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openabg-cli-images-"));
  const paths: string[] = [];
