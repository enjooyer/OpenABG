...
import { OpenABGApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = OpenABGApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("openabg-app") as OpenABGApp;
...
beforeEach(() => {
  OpenABGApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__OPENABG_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  OpenABGApp.prototype.connect = originalConnect;
  window.__OPENABG_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openabg/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openabg");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openabg/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENABG_CONTROL_UI_BASE_PATH__ = "/openabg";
    const app = mountApp("/openabg/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openabg");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openabg/sessions");
  });
